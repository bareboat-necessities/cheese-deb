...
docker exec --privileged -ti $DOCKER_CONTAINER_ID cp /etc/apt/sources.list /etc/apt/sources.list~; sed -Ei 's/^# deb-src /deb-src /' /etc/apt/sources.list; apt-get update
